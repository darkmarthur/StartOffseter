...
class AudioProcessor:
    ...
    @staticmethod
    def calculate_bpm(file_path, log_text):
        try:
            fs, data = wav.read(file_path)
            Utility.log_message(log_text, f"Loaded audio file with sample rate: {fs}", "blue")

            if len(data.shape) > 1:  # Convert to mono by averaging channels
                data = np.mean(data, axis=1)
            Utility.log_message(log_text, "Converted to mono", "blue")

            # Normalize the data
            data = data / np.max(np.abs(data)).astype(np.float64)

            # Apply bandpass filter to isolate rhythmic elements
            filtered_data = AudioProcessor.bandpass_filter(data, lowcut=30, highcut=160, fs=fs, order=4)
            Utility.log_message(log_text, "Applied bandpass filter", "blue")

            # Normalize the filtered data to avoid overflow issues
            filtered_data = filtered_data / np.max(np.abs(filtered_data))

            # Calculate the energy of the signal safely
            energy = np.square(filtered_data)  # Square the signal
            Utility.log_message(log_text, "Calculated energy of the filtered signal", "blue")

            # Smooth the energy signal to reduce noise
            smoothed_energy = AudioProcessor.smooth_signal(energy, window_len=50)
            Utility.log_message(log_text, "Smoothed the energy signal", "blue")

            # Adjust the height and distance for peak detection
            peak_threshold = np.mean(smoothed_energy) * 1.2
            distance_between_peaks = fs // 4

            peaks, _ = find_peaks(smoothed_energy, height=peak_threshold, distance=distance_between_peaks)
            Utility.log_message(log_text, f"Detected {len(peaks)} peaks", "blue")

            # Check if any peaks were found
            if len(peaks) == 0:
                Utility.log_message(log_text, "No peaks detected, defaulting to 120 BPM", "red")
                return 120.0

            # Calculate intervals between peaks (in seconds)
            intervals = np.diff(peaks) / fs
            if len(intervals) == 0:
                Utility.log_message(log_text, "No intervals detected, defaulting to 120 BPM", "red")
                return 120.0
            ...
            Utility.log_message(log_text, f"Calculated BPM: {bpm:.2f}", "green")
            return bpm
    ...
    def calculate_bpm(file_path, log_text):
        try:
            ...
            # Calculate the energy of the signal safely
            energy = np.square(filtered_data)  # Square the signal instead of absolute to avoid redundant operations
            Utility.log_message(log_text, "Calculated energy of the filtered signal", "blue")

            # Adjust the height and distance for peak detection
            peak_threshold = np.mean(energy) * 1.5
            distance_between_peaks = fs // 4
            
            peaks, _ = find_peaks(energy, height=peak_threshold, distance=distance_between_peaks)
            Utility.log_message(log_text, f"Detected {len(peaks)} peaks", "blue")
...
class FileHandler:
    ...
    @staticmethod
    def process_file(file_path, log_text, rename_file_var, bpm_option, beats_entry, entries, fix_wav_hex_var, track_type_var, loading_label):
        try:
            bpm_value = None
            if bpm_option.get() == 'auto':
                bpm_value = AudioProcessor.calculate_bpm(file_path, log_text)
            else:
            ...
            # Stop the loading spinner after processing is complete
            Utility.stop_loading(loading_label)
...
class StartOffseterApp(tk.Tk):
    ...
    def open_file_dialog(self):
        ...
        if file_path:
            ...
            Thread(target=FileHandler.process_file, args=(file_path, self.log_text, self.rename_file_var, self.bpm_option, self.beats_entry, self.entries, self.fix_wav_hex_var, self.track_type_var, self.loading_label)).start()
